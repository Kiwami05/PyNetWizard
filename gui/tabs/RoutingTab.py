...
class RoutingTab(QWidget):
    ...
    def _make_ospf_tab(self):
        ...
        f.addRow("Process ID:", self.ospf_pid)
        ...
        btn_add.clicked.connect(self._ospf_add_update)
        btn_del = QPushButton("Delete")
    ...
    def _ospf_add_update(self):
        pid = self.ospf_pid.text().strip()
        net = self.ospf_network.text().strip()
        w = self.ospf_wild.text().strip()
        area = self.ospf_area.text().strip()

        if pid != "1":
            QMessageBox.warning(self, "Błąd", "Obsługujemy tylko process 1.")
            self.ospf_pid.setText("1")
            return

        if not (is_valid_ip(net) and is_valid_wildcard(w)):
        ...
        if row is None:
            ...
            self.ospf_table.setItem(r, 2, QTableWidgetItem(w))
            self.ospf_table.setItem(r, 3, QTableWidgetItem(area))

            self._enqueue(
                [
                    f"router ospf {pid}",
                    f" network {net} {w} area {area}",
                    " exit",
                ]
            )
            ...
            old_w = self.ospf_table.item(row, 2).text()
            old_area = self.ospf_table.item(row, 3).text()

            if old_pid == pid and old_net == net and old_w == w and old_area == area:
                return

            cmds = [
                f"router ospf {old_pid}",
                f" no network {old_net} {old_w} area {old_area}",
                " exit",
                f"router ospf {pid}",
                f" network {net} {w} area {area}",
                " exit",
            ...
            self.ospf_table.setItem(row, 2, QTableWidgetItem(w))
    ...
    def sync_from_config(self, conf: ParsedConfig):
        ...
        try:
            ...
            for o in conf.routing.ospf:
                ...
                self.ospf_table.setItem(row, 0, QTableWidgetItem("1"))
                self.ospf_table.setItem(row, 1, QTableWidgetItem(o["network"]))
                self.ospf_table.setItem(row, 2, QTableWidgetItem(o["wildcard"]))
                self.ospf_table.setItem(row, 3, QTableWidgetItem(o["area"]))
